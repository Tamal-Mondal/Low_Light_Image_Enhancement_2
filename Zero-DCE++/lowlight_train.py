...
def train(config):
    ...
    if config.load_pretrain == True:
        print("Loading pre-traied model: ", config.pretrain_dir)
    ...
    print("\nNumber of epochs: {} and Number of iterations per epoch: {}".format(config.num_epochs, len(train_dataset) // config.train_batch_size))
    for epoch in range(config.num_epochs):
        ...
        for iteration, img_lowlight in enumerate(train_loader):
            ...
            if ((iteration + 1) % config.display_iter) == 0:
                print("Loss at iteration", iteration + 1, "of epoch", epoch, ":", loss.item())

            if ((iteration + 1) % config.snapshot_iter) == 0:
                torch.save(DCE_net.state_dict(), config.snapshots_folder + "Epoch" + str(epoch) + ".pth")

        end_time = time.time() - start
        tbwriter.add_scalar(f"training_loss:", loss_ep / len(train_loader), epoch)
        tbwriter.add_scalar(f"training_time:", end_time, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--snapshots_folder", type=str, default="checkpoints/attention_pool_no_bn_bias/")
