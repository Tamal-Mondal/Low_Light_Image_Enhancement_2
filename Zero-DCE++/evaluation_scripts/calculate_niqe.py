...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    print(img.shape)
    img2 = np.array(Image.fromarray(img).resize(size = (int(img.shape[0]/2), int(img.shape[1]/2)), resample = PIL.Image.BICUBIC))
...
if __name__ == "__main__":
    ...
    folder_location = "/raid/cs21mtech12001/DL_project/Low_Light_Image_Enhancement_2/Zero-DCE++/data/result_Zero_DCE++/attention_4_layers"
    images = glob.glob(folder_location + "/*") 
    total_niqe = 0
    for i in range(len(images)):
        total_niqe += niqe(np.array(Image.open(images[i]).convert('LA'))[:,:,0])
    print("Average NIQE is: ", total_niqe/len(images))
